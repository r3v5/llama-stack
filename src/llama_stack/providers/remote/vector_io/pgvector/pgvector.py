...
class PGVectorIndex(EmbeddingIndex):
        ...
        try:
            with self.conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                # Create the index only after the table has some data (https://github.com/pgvector/pgvector?tab=readme-ov-file#ivfflat)
                    ...
                    and not self.check_conflicting_vector_index_exists(cur)
                ):
                    log.info(
    ...
    async def create_hnsw_vector_index(self, cur: cursor) -> None:
        ...
        if self.check_conflicting_vector_index_exists(cur):
            return
    ...
    def check_conflicting_vector_index_exists(self, cur: cursor) -> bool:
        """Check if index of another type has already been created for the table to prevent the conflict
